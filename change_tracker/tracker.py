...
def histCompare(a, b):
    hsv_base = cv2.cvtColor(a, cv2.COLOR_BGR2HSV)
    hsv_test1 = cv2.cvtColor(b, cv2.COLOR_BGR2HSV)
    h_bins = 50
    s_bins = 60
    ...
    return cv2.compareHist(hist_base, hist_test1, 3)
...
class SimilarityModel():
    ...
    def __call__(self, first, second): 
        ...
        return (self._model.predict([first, second])[0] + histCompare(first[0], second[0]))/2.
...
class ChangeTracker():
    is_initalized = False
    def __init__(self, maxDisappeared=3, iou_thresh =.4, similarity_tresh=0.5, past_similarity=3):
        self.nextObjectID = 0
