...
class GenericObjSessionData_TfCreator:
    def __init__(self, session_dir):
        ...
        self.max_ann = 0
        self.n_boxes = 0
        self.dif_folder = os.path.join(session_dir, "dif")
        self.current_empty = 0
        self.max_empty = 1
        ...
        if len(imgs_names) - len(dif_names) > 1:
            ...
            for img_name in imgs_names:
                ...
                if prev is not None and not os.path.exists(dif_path):
                    dif_img = Image.fromarray(diff(im, prev))
                    dif_img.save(os.path.join(self.dif_folder, img_name))
                prev = im
        print ("creating tf records")

        for img_name in imgs_names[1:]:
            tf_exmapl = self.dict_to_tf_examples(img_name)
            if tf_exmapl is not None:
                self.tf_examples.append(tf_exmapl)
    ...
    def dict_to_tf_examples(self,
        ...
        if len(new_ann) > 0 or self.current_empty < self.max_empty:
            if len(new_ann) > 0:
                self.current_empty = 0
            else:
                self.current_empty +=  1
            for obj in new_ann:
